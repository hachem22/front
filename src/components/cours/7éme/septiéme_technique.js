...
function SeptiemeTechnique() {
  ...
  useEffect(() => {
    const fetchCoursData = async () => {
      try {
        const response = await fetch("http://localhost:3000/cours/7éme/Technique");
    ...
    const fetchProgression = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/progression/Technique/7eme/${eleveId}`);
    ...
    if (selectedVideo && eleveId) {
      if (!watchedVideos.some(watchedVideo => watchedVideo === selectedVideo.videoUrl)) {
        ...
        .then(response => {
          ...
          const progressionTechnique = (updatedWatchedVideos.length / cours.length) * 100;

          axios.put(`http://localhost:3000/eleves/${eleveId}/${matricule}/progression/Technique`, {
  ...
  return (
    <div>
      <Navbarsept/>
      <h1 style={{ marginBottom: '-4rem',marginTop:'-42rem' }}>Playlist des cours de Technique de 7ème année</h1>
...
export default SeptiemeTechnique;
